...
export function Settings({ settings, logs, onSave, onBack }: { settings: AppSettings; logs: DailyLog[]; onSave: (settings: AppSettings) => Promise<void>; onBack: () => void }) {
  ...
  const startPlan = async () => {
    ...
    const next = { ...form, startingWeightKg: latestWeight.weightKg, goalWeightKg: 85, planStartDate, goalDate: addCalendarYear(planStartDate) }
    setForm(next)
    await save(next)
  }
  ...
  return <>
    ...
    <form onSubmit={submit} className="settings-card"><div className="checkin-card-heading"><span className="section-icon"><Icon name="settings" className="h-5 w-5" /></span><div><h2>Your personal baseline</h2><p>Keep your height and exact goal current; the saved plan baseline stays put until you deliberately restart it.</p></div></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Height (cm)"><input type="number" min="100" max="250" value={form.heightCm} onChange={(event) => update('heightCm', numberOr(event.target.value, form.heightCm))} /></Field><Field label="Goal weight (kg)"><input type="number" min="30" max="300" step="0.1" value={form.goalWeightKg} onChange={(event) => update('goalWeightKg', numberOr(event.target.value, form.goalWeightKg))} /></Field></div>{error && <div className="notice notice-error mt-5" role="alert">{error}</div>}{saved && <div className="notice notice-good mt-5" role="status">✓ Your personal settings are saved.</div>}<button className="btn mt-5" disabled={saving}>{saving ? 'Saving settings…' : 'Save settings'}</button></form>
    <section className="plan-overview"><div><p className="eyebrow">Your plan right now</p><h2>Month {month} · {goal}+ hour fasting goal</h2><p>From {fmtNum(form.startingWeightKg)} kg on {fmtDate(form.planStartDate)}, your exact target is {fmtNum(form.goalWeightKg)} kg by {fmtDate(form.goalDate)}.</p></div><div className="mt-5"><button type="button" className="btn-secondary" disabled={saving || !latestWeight} onClick={() => void startPlan()}>{saving ? 'Saving plan…' : 'Start/restart 85 kg plan'}</button><p className="mt-3 text-xs text-emerald-50/80">{latestWeight ? `This will snapshot your latest weigh-in of ${fmtNum(latestWeight.weightKg)} kg from ${fmtDate(latestWeight.date)} and set its one-year deadline.` : 'Log a morning weight before starting a new plan.'}</p></div></section>
