...
const eatingWindowPresets = [
  { label: '12–6 · 18h', start: '12:00', end: '18:00' },
  { label: '12–8 · 16h', start: '12:00', end: '20:00' },
  { label: '10–6 · 16h', start: '10:00', end: '18:00' },
  { label: '9–5 · 16h', start: '09:00', end: '17:00' },
  { label: '8–4 · 16h', start: '08:00', end: '16:00' },
]
...
export const fastingHoursForWindow = (start: string, end: string) => {
  const startMinutes = timeToMinutes(start); const endMinutes = timeToMinutes(end)
  if (startMinutes == null || endMinutes == null || endMinutes <= startMinutes) return null
  return Math.round((24 - (endMinutes - startMinutes) / 60) * 10) / 10
}
...
export function CheckIn({ logs, selectedDate, onDateChange, onSave }: { logs: DailyLog[]; selectedDate: string; onDateChange: (date: string) => void; onSave: (log: DailyLog) => Promise<DailyLog> }) {
  ...
  const today = new Date(); const latestDate = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`
  ...
  useEffect(() => {
    const next = existing ? { ...existing } : emptyLog(selectedDate)
    setForm({ ...next, fastingHours: fastingHoursForWindow(next.eatingWindowStart, next.eatingWindowEnd) })
  ...
  const updateEatingWindow = (start: string, end: string) => setForm((current) => ({ ...current, eatingWindowStart: start, eatingWindowEnd: end, fastingHours: fastingHoursForWindow(start, end) }))
  const changeDate = (date: string) => { onDateChange(date) }
  const fastingHours = fastingHoursForWindow(form.eatingWindowStart, form.eatingWindowEnd)
  const windowError = fastingHours == null ? 'End time must be later than start time. Overnight windows are not supported yet.' : ''
  const isToday = form.date === latestDate
  const submit = async (event: React.FormEvent) => {
    event.preventDefault()
    if (fastingHours == null) { setError(windowError); return }
    setSaving(true); setError('')
    try {
      const now = new Date().toISOString()
      const saved = await onSave({ ...form, fastingHours, id: existing?.id ?? form.id, createdAt: existing?.createdAt ?? form.createdAt, updatedAt: now })
  ...
  return <>
    ...
    <form onSubmit={submit} className="space-y-5 pb-20 md:pb-0">
      <section className="checkin-card checkin-highlight"><div className="checkin-card-heading"><span className="section-icon"><Icon name="calendar" className="h-5 w-5" /></span><div><h2>Today’s anchor</h2><p>Start with the two details that make progress visible.</p></div></div><div className="grid gap-4 sm:grid-cols-2"><Field label="Date"><input type="date" value={form.date} max={latestDate} onChange={(event) => changeDate(event.target.value)} required /></Field><Field label="Morning weight (kg)" hint="Optional, but useful for a clear trend."><input type="number" min="30" max="300" step="0.1" value={form.weightKg ?? ''} onChange={(event) => update('weightKg', toNumber(event.target.value))} placeholder="Example: 109.5" /></Field></div></section>

      <section className="checkin-card"><div className="checkin-card-heading"><span className="section-icon"><Icon name="trend" className="h-5 w-5" /></span><div><h2>Fasting & meals</h2><p>Capture the rhythm, not a perfect food diary.</p></div></div><div className="eating-window"><div><span className="field-label">Quick window</span><div className="window-presets" aria-label="Eating window presets">{eatingWindowPresets.map((preset) => <button key={preset.label} type="button" className={form.eatingWindowStart === preset.start && form.eatingWindowEnd === preset.end ? 'is-selected' : ''} aria-pressed={form.eatingWindowStart === preset.start && form.eatingWindowEnd === preset.end} onClick={() => updateEatingWindow(preset.start, preset.end)}>{preset.label}</button>)}</div></div><div className="grid gap-4 sm:grid-cols-3"><Field label="Eating window starts"><input type="time" value={form.eatingWindowStart} onChange={(event) => updateEatingWindow(event.target.value, form.eatingWindowEnd)} />{isToday && <button className="time-action" type="button" onClick={() => updateEatingWindow(localTime(), form.eatingWindowEnd)}>Start now</button>}</Field><Field label="Eating window ends"><input type="time" value={form.eatingWindowEnd} onChange={(event) => updateEatingWindow(form.eatingWindowStart, event.target.value)} />{isToday && <button className="time-action" type="button" onClick={() => updateEatingWindow(form.eatingWindowStart, localTime())}>End now</button>}</Field><div><span className="field-label">Fasting hours</span><output className="fasting-result" aria-live="polite">{fastingHours == null ? '—' : `${fastingHours} h`}</output><span className="field-hint">Calculated from your eating window.</span></div></div>{windowError && <p className="window-error" role="alert">{windowError}</p>}</div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field wide label="First meal"><textarea value={form.firstMeal} onChange={(event) => update('firstMeal', event.target.value)} placeholder="What felt satisfying?" /></Field><Field wide label="Second meal"><textarea value={form.secondMeal} onChange={(event) => update('secondMeal', event.target.value)} placeholder="Add anything useful to remember." /></Field></div><div className="mt-5 grid gap-4 sm:grid-cols-2"><Toggle label="Any snacks?" checked={form.snacks} onChange={(value) => update('snacks', value)} /><Toggle label="Any late-night eating?" checked={form.lateNightEating} onChange={(value) => update('lateNightEating', value)} /><Toggle label="Any sugary drink?" checked={form.sugaryDrink} onChange={(value) => update('sugaryDrink', value)} /><Field label="Refined carbs"><select value={form.refinedCarbs} onChange={(event) => update('refinedCarbs', event.target.value as RefinedCarbs)}><option value="none">None</option><option value="low">Low</option><option value="medium">Medium</option><option value="high">High</option></select></Field></div></section>
      ...
      <div className="sticky-save"><button className="btn w-full sm:w-auto" disabled={saving || fastingHours == null}>{saving ? 'Saving your check-in…' : existing ? 'Save changes' : 'Save today'}</button></div>
    </form>
  </>
}
